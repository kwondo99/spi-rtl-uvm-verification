git : https://github.com/kwondo99/spi-rtl-uvm-verification

# SPI Protocol Design & UVM Verification

SPI(Serial Peripheral Interface) Protocol을 RTL로 설계하고, **UVM 기반 검증 환경**을 구축하여 **Scoreboard**와 **Coverage**를 통해 동작을 검증한 프로젝트이다.
SPI **Master**와 **Slave**를 각각 독립적인 UVM 환경으로 검증하였다.
...
SPI Master/Slave를 직접 설계하고, UVM으로 각각 검증한다.
...
Scoreboard로 송수신 데이터의 정확성을, Coverage로 검증 범위를 측정한다.
...
4개의 신호선을 이용해 마스터와 슬레이브 간 데이터를 동시에 주고받는(full-duplex) 동기식 직렬 통신 프로토콜이다.
...
Master를 DUT로 두고, Driver가 `fork...join`으로 **master 자극**과 **가상 slave 응답(MISO)** 을 동시에 생성한다.
가상 slave는 SPI Mode(CPOL/CPHA)에 따라 MISO 데이터 인가 시점을 다르게 처리한다.
...
Slave를 DUT로 두고, Driver가 **가상 master 신호(sclk, mosi, ss_n)** 를 직접 생성한다.
**Mode 0 (CPOL=0, CPHA=0)** 만 지원하며, 고정 `CLK_DIV = 4`로 클럭 delay를 정의한다.
